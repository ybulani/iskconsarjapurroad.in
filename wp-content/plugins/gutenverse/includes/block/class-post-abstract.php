...
abstract class Post_Abstract extends Block_Abstract {
	...
	public function format_date( $post ) {
		...
		if ( 'both' === $date_type ) {
			$output = gutenverse_get_post_date( $post, $this->attributes['metaDateFormat'], 'published', $this->attributes['metaDateFormatCustom'] );
			$output = $output . esc_html__( ' - Updated on ', 'gutenverse' );
			$output = $output . gutenverse_get_post_date( $post, $this->attributes['metaDateFormat'], 'modified', $this->attributes['metaDateFormatCustom'] );
		} else {
			$output = gutenverse_get_post_date( $post, $this->attributes['metaDateFormat'], $date_type, $this->attributes['metaDateFormatCustom'] );
		}
